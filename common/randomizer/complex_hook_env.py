...
class ComplexHookEnv(gym.Env):
    ...
    def _randomize_block_mass(self):
        ...
        self._env.sim.model.body_mass[-2] = block_mass

    def _randomize_hook_mass(self):
        hook_mass = self.dimensions[1].current_value
        self._env.sim.model.body_mass[-1] = hook_mass

    def _randomize_friction(self):
        current_friction = self.dimensions[2].current_value
        for i in range(len(self._env.sim.model.geom_friction)):
            self._env.sim.model.geom_friction[i] = [current_friction, 5.e-3, 1e-4]
