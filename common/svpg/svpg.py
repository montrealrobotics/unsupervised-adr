...
class SVPG:
    ...
    """
    def __init__(self, nparticles, nparams, max_step_length, svpg_rollout_length, svpg_horizon, temperature, discrete=False, kld_coefficient=0.0):
        self.particles = []
        ...
        self.last_states = np.random.uniform(0, 1, (self.nparticles, self.nparams))
        self.timesteps = np.zeros(self.nparticles)
    ...
    def _Kxx_dxKxx(self, X):
        ...
        h = np.sqrt(0.5 * h / np.log(self.nparticles+1))
    ...
    def step(self):
        ...
        """
        self.simulation_instances = np.zeros((self.nparticles, self.svpg_rollout_length, self.nparams))

        # Store the values of each state - for advantage estimation
        self.values = torch.zeros((self.nparticles, self.svpg_rollout_length, 1)).float().to(device)
        # Store the last states for each particle (calculating rewards)
        self.masks = np.ones((self.nparticles, self.svpg_rollout_length))
    ...
    def train(self, simulator_rewards):
        ...
        grad_theta = (grad_logp + dxKxx) / self.nparticles
