...
parser.add_argument('--sp-gamma', type=float, default=0.01, metavar='N',
                    help='Self Play gamma')
parser.add_argument('--sp-percent', type=float, default=1.0, metavar='N',
                    help='Self Play percentage')
...
def check_closeness(state, goal):
    dist =  np.linalg.norm(state[:2] - goal[:2]) ** 2
    print(dist, state[:2], goal[:2], 'dist')
...
def evaluate_policy(env, policy):
    ...
    while nepisodes < N_ROLLOUTS:        
        with torch.no_grad():
            ...
            residual_action = policy.select_action(full_state, deterministic=False, save_log_probs=False)[0]
    ...
    while timesteps < total_timesteps:
        if np.random.random() < args.sp_percent:
            ...
            while not alice_done and time_alice <= MAX_TIMESTEPS:
                ...
                alice_done = env_done or alice_signal and time_alice > 10
