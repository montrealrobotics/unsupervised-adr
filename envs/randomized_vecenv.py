...
def make_vec_envs(env_name, seed, num_processes, randomize_on_done=True):
    envs = [make_env(env_name, seed, i)
            for i in range(num_processes)]
    envs = RandomizedSubprocVecEnv(envs, randomize_on_done)
    return envs


def worker(remote, parent_remote, env_fn_wrapper, randomize_on_done):
    parent_remote.close()
    ...
    try:
        while True:
            ...
            if cmd == 'step':
                ...
                if done: # TODO: Not the case in our setup!
                    if randomize_on_done:
                        env.randomize([-1])
                    ob = env.reset()
...
class RandomizedSubprocVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, randomize_on_done, spaces=None):
        """
        Arguments:
        ...
        self.ps = [Process(target=worker, args=(work_remote, remote, CloudpickleWrapper(env_fn), randomize_on_done))
