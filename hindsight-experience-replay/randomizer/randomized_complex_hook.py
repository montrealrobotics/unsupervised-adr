...
class ComplexHookSingleObjectEnv(fetch_env.FetchEnv, utils.EzPickle):
    ...
    def _randomize_block_mass(self, dimensions):
        block_mass = dimensions[0].current_value
        self.sim.model.body_mass[-2] = block_mass

    def _randomize_hook_mass(self,dimensions):
        hook_mass = dimensions[1].current_value
        self.sim.model.body_mass[-1] = hook_mass

    def _randomize_friction(self, dimensions):
        current_friction = dimensions[2].current_value
        for i in range(len(self.sim.model.geom_friction)):
            self.sim.model.geom_friction[i] = [current_friction, 5.e-3, 1e-4]

    def update_randomized_params(self, dimensions):
        self._randomize_block_mass(dimensions)
        self._randomize_hook_mass(dimensions)
        self._randomize_friction(dimensions)
...
class ComplexHookEnv(gym.Env):
    def __init__(self, train=True, num_scenes=100, scene_offset=0, subenv_type='normal', **kwargs):
        if train:
            xml_files = list(glob.glob(os.path.join(DIR_PATH, 'assets_residual', 'fetch_complex_objects', 'fetch',
    ...
    def update_randomized_params(self):
        self._env.update_randomized_params(self.dimensions)
...
class ResidualComplexHookEnv(ComplexHookEnv):
    def __init__(self, **kwargs):
        ComplexHookEnv.__init__(self, train=True, subenv_type='residual', **kwargs)


class TwoFrameResidualComplexHookEnv(ComplexHookEnv):
    def __init__(self, **kwargs):
        ComplexHookEnv.__init__(self, train=True, subenv_type='twoframe_residual', **kwargs)
